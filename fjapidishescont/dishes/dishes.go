...
import (
	helper "fjapidishescont/helper"
	"fmt"
	"log"

	dishes "fjapidishescont/models"
...
func Getall(redisclient *redis.Client) []dishes.Dish {
	...
	fmt.Println("database.Location: " + database.Location)
	fmt.Println("database.Collection: " + database.Collection)

	session, err := mgo.Dial(database.Location)

	if err != nil {
		fmt.Println("error: " + err.Error())
		fmt.Println("error is below: ")
		// panic(err)
		fmt.Println(err)
		log.Fatal(err)
		return results
